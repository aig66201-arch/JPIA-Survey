...
function doGet(e) {
  ...
  if (action === 'getSubmittedNames') {
    return json_(getSubmittedNames_());
  }
  return json_({success:true,message:'BSAIS 3A Survey API is running.'});
}
...
function submitSurvey(data) {
  ...
  const middleInitial = clean_(data.middleInitial).replace(/[^A-Za-z]/g,'').slice(0,1).toUpperCase();
  ...
  if (!lastName || !firstName || !/^[A-Za-z]$/.test(middleInitial))
    throw new Error('Last Name, First Name, and a one-letter Middle Initial are required.');
  ...
  try {
    ...
    // Preserve existing Google Sheets dropdown/data-validation rules on the new row.
    copyDataValidationToNewRow_(sheet, row);

    const databaseLastName = lastName + (extensionName ? ' ' + extensionName : '');
    ...
    sheet.getRange(row,7).setFormula('=E' + row + '&" "&F' + row + '&" "&D' + row);
    ...
    sortBSAIS3A_();
    sendConfirmationEmail_(email, firstName, sheet.getRange(row,7).getDisplayValue());

    return {success:true,message:'Your survey was submitted successfully. A confirmation email has been sent to your email address.'};
  } finally {
    lock.releaseLock();
  }
}
...
function copyDataValidationToNewRow_(sheet, targetRow) {
  ...
  for (let r = lastExisting; r >= FIRST_DATA_ROW; r--) {
    if (candidates.indexOf(r) === -1) candidates.push(r);
  }

  let source = null;
  for (const r of candidates) {
    const rules = sheet.getRange(r,3,1,width).getDataValidations()[0];
    if (rules.some(rule => rule !== null)) { source = rules; break; }
  }
  if (source) sheet.getRange(targetRow,3,1,width).setDataValidations([source]);
}
...
function getSubmittedNames_() {
  ...
  const values = sheet.getRange(FIRST_DATA_ROW,4,lastRow-FIRST_DATA_ROW+1,4).getDisplayValues();
  return values.filter(r => r[0] || r[1] || r[2]).map(r => r[3] || [r[1],r[2],r[0]].filter(Boolean).join(' ')).sort((a,b)=>a.localeCompare(b));
}
...
function sortBSAIS3A_() {
  ...
  const lastRow = sheet.getLastRow();
  if (lastRow < FIRST_DATA_ROW) return;
  sheet.getRange(FIRST_DATA_ROW,3,lastRow-FIRST_DATA_ROW+1,16).sort([{column:4,ascending:true}]);
}
...
function normalizeExtension_(v) { const x=clean_(v).replace(/[.]+$/,''); return x ? x+'.' : ''; }
